...
func solvePuzzle(input: [String]) -> Int {
    ...
    let seedPairs = input[0].split(separator: ": ")[1].split(separator: " ").map { return Int($0)! }
    var seeds: [[Int]] = []

    var i: Int = 0
    while i < seedPairs.count {
        seeds.append([seedPairs[i], seedPairs[i + 1]])
        i += 2
    }
