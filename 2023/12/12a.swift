...
func solvePuzzle(input: [String]) -> Int {
    ...
    for line in input {
        let symbolGroups = line.split(separator: " ")[0]
        let numbers = line.split(separator: " ")[1].split(separator: ",").map { return Int($0)! }

        let count = countArrangements(symbolGroups: String(symbolGroups), numbers: numbers)
