...
let testSolution: Int = 5905

let textFile: String = try String(contentsOfFile: "./puzzleInput.txt")
let puzzleInput: [String] = textFile.components(separatedBy: "\n")

let cardOrder: [Character] = ["A", "K", "Q", "T", "9", "8", "7", "6", "5", "4", "3", "2", "J"]
...
struct Hand: Hashable {
    ...
    var worth: Int {
        ...
        for cardSum in cardSums {
            if (cardSums.contains{ $0.key == "J"} && cardSum.key != "J") {
                if (cardSum.value + cardSums["J"]! == 5) {                              // Five of a kind with Jokers
                    tmpWorth = max(tmpWorth, 7)
                } else if (cardSum.value + cardSums["J"]! == 4) {                       // Four of a kind with Jokers
                    tmpWorth = max(tmpWorth, 6)
                } else if (cardSums.contains{ $0.value + cardSums["J"]! == 3 && cardSums.count == 3}) {  // Full house with Jokers
                    tmpWorth = max(tmpWorth, 5)
                } else if (cardSum.value + cardSums["J"]! == 3) {                       // Three of a kind with Jokers
                    tmpWorth = max(tmpWorth, 4)
                } else if (cardSums.contains{ $0.value + cardSums["J"]! == 2 } && cardSums.count == 4) { // Two pair with Jokers
                    tmpWorth = max(tmpWorth, 3)
                } else {                                                                // One pair (with Jokers there is always one pair)
